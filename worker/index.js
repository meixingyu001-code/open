...
function scriptedReply(history) {
  ...
  // "今天吃什么/现在在哪"这类问题没有真答案(语料是历史的,不是实时的),模型会拿旧材料
  // 冒充当下事实,还编细节("新米""藜麦"这种语料里根本没有的东西)。这条比 prompt 里
  // "不许编"管用——拦在确定性层,不让它有机会现编。
  const realtimeFactQuestion = /(今天|现在|刚才|刚刚|昨天晚上|此刻).{0,6}(吃|穿|在哪|几点|干嘛|干什么|在干|睡了|起床)/.test(last);
  if (realtimeFactQuestion) {
    return "这个我说不好——我不是实时同步的，她今天/现在具体在干嘛、吃什么，我这边不知道。这种当下的事得问她本人；如果你想聊聊她一直以来对吃饭、生活节奏这些事的态度，我们可以聊那个。";
  }
